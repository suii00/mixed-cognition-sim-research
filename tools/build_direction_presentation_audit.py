#!/usr/bin/env python3
"""Build or verify the frozen twelve-cell direction-presentation audit."""
...
DEFAULT_PLAN = REPO_ROOT / "configs" / "direction_presentation_audit_v2" / "plan.json"


def _write_once(path: Path, data: bytes) -> None:
    if path.exists() or path.is_symlink():
        raise DirectionPresentationAuditError(
            f"refusing to overwrite generated artifact: {path}"
        )
...
def build(plan_path: Path) -> None:
    ...
    if manifest_path.exists() or manifest_path.is_symlink():
        raise DirectionPresentationAuditError(
            f"refusing to overwrite manifest: {manifest_path}"
        )
    if config_root.exists() or config_root.is_symlink():
        raise DirectionPresentationAuditError(
            f"refusing to overwrite config directory: {config_root}"
        )
    for relative_path, data in configs.items():
        _write_once(root / relative_path, data)
    _write_once(manifest_path, json_bytes(manifest))
    print(f"wrote {len(configs)} configs and manifest under {root.relative_to(REPO_ROOT)}")
...
def check(plan_path: Path) -> None:
    ...
    if not manifest_path.is_file() or manifest_path.read_bytes() != json_bytes(manifest):
        raise DirectionPresentationAuditError(
            "generated manifest differs from deterministic builder"
        )
    ...
    if observed_paths != expected_paths:
        raise DirectionPresentationAuditError(
            "generated config file set differs from manifest"
        )
    for relative_path, data in configs.items():
        if (root / relative_path).read_bytes() != data:
            raise DirectionPresentationAuditError(
                f"generated config differs: {relative_path}"
            )
    print(f"verified {len(configs)} configs and manifest under {root.relative_to(REPO_ROOT)}")
