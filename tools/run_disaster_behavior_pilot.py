#!/usr/bin/env python3
"""Run the frozen six-run descriptive follow-up with a nine-request schema gate."""
...
from tools.build_disaster_behavior_pilot import (
    BATCH_ID, CALLS_PER_RUN, OUTPUT_DIR, PLANNED_CALLS, PROTOCOL_VERSION,
...
RUNTIME_PREFIX = "disaster-pilot-runtime-"
...
def load_inputs():
    # Byte equality checks every declared config and the manifest before any output.
    ...
    for row in manifest["rows"]:
        ...
        bloc = config["blocs"][0]
        models[bloc["name"]] = bloc
    if len(configs) != 6 or set(models) != set(MODEL_ORDER):
        raise PublicVllmError("expected six runs and three models")
...
def check_run(run_dir, config, specs, source_sha):
    ...
    counts_ok = (
        meta.get("logical_llm_calls") == CALLS_PER_RUN and meta.get("http_attempts") == CALLS_PER_RUN
...
def run(args):
    ...
    if args.contract_only:
        print("PASS: six frozen configs, 2880 experiment requests, nine gate requests, four GPUs")
        return 0
    if os.name != "posix":
    ...
    servers = []
    simulations = []
    rows = [{"run_id": run_id, "status": "not_started"} for run_id in configs]
    attempts = []
    error_code = None
    process_cleanup = False
    gpu_release = False
    result = {
        "schema_version": "disaster-behavior-pilot-verification-v1.0.0", "batch_id": batch,
        ...
        with tempfile.TemporaryDirectory(prefix=RUNTIME_PREFIX) as tmp:
            ...
            try:
                ...
                for ordinal, (run_id, config) in enumerate(configs.items()):
                    ...
                    row_runtime.mkdir()
                    used_specs = [s for s in specs if s.endpoint_id == config["blocs"][0]["endpoint_id"]]
                    ...
                    rows[ordinal] = check_run(run_dir, config, specs, args.source_git_sha)
                    rows[ordinal]["exit_code"] = return_code
                    print(f"run {ordinal + 1}/6: {rows[ordinal]['status']}", flush=True)
    ...
    for index, row in enumerate(rows):
        ...
        if row["status"] == "not_started" and meta_path.is_file():
            ...
            rows[index] = {"run_id": row["run_id"], "status": meta.get("status"),
    ...
    print(f"PASS: six runs completed; evidence={final_evidence.name}", flush=True)
    return 0
...
def main(argv=None):
    ...
    try:
        ...
        print(f"ERROR: {type(error).__name__}: follow-up execution rejected", file=sys.stderr)
        return 2
