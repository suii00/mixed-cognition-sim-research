...
from tools import run_warning_retention_study as runner


class WarningRetentionStudyRunnerTests(unittest.TestCase):
    ...
    def test_frozen_paired_rows_derive_equal_request_counts(self):
    ...
    def test_run_and_batch_collisions_preserve_existing_bytes(self):
        for location in ("stage", "run"):
            with self.subTest(location=location), tempfile.TemporaryDirectory() as temp, \
                ...
                with self.assertRaises(runner.PublicVllmError):
                    runner.checked_output_paths(runner.BATCH_ID, ["one"], repo_root=root)
    ...
    def test_complete_batch_gates_nine_probes_then_all_six_paired_runs(self):
        self.mock_execution()
    ...
    def test_cleanup_failure_preserves_six_completed_runs_without_promotion(self):
        self.mock_execution("cleanup")

    def test_missing_terminal_metadata_is_retained_as_incomplete(self):
        self.mock_execution("missing_terminal")

    def test_source_gate_rejects_dirty_or_unfrozen_source(self):
        for outputs in (("b" * 40, ""), ("a" * 40, " M engine/sim.py")):
            with mock.patch.object(runner.subprocess, "run", side_effect=[SimpleNamespace(stdout=s) for s in outputs]):
                with self.assertRaisesRegex(runner.PublicVllmError, "frozen clean"):
                    runner.source_gate("a" * 40)
        with mock.patch.object(runner.subprocess, "run") as git:
            with self.assertRaises(runner.PublicVllmError):
                runner.source_gate("not-a-commit")
        git.assert_not_called()

    def test_preflight_checks_without_allocating_or_creating_outputs(self):
        ...
        with tempfile.TemporaryDirectory() as temp, ExitStack() as stack:
            ...
            for name, value in {"source_gate": None, "check_installed_runtime": lock["packages"],
                                "attach_snapshots": specs, "create_gpu_guard": object(),
                                "predicted_runtime_ipc_path_bytes": 68, "ports_are_free": True}.items():
                stack.enter_context(mock.patch.object(runner, name, return_value=value))
            allocate = stack.enter_context(mock.patch.object(runner.tempfile, "TemporaryDirectory"))
            launch = stack.enter_context(mock.patch.object(runner, "start_servers_sequentially"))
            self.assertEqual(runner.run(self.args(preflight_only=True)), 0)
    ...
    def test_promotion_error_rolls_back_prior_moves_without_editing_raw(self):
        with tempfile.TemporaryDirectory() as temp, mock.patch.object(runner, "REPO_ROOT", Path(temp)):
            ...
            with mock.patch.object(Path, "rename", fail_second), self.assertRaises(runner.PublicVllmError):
                runner.promote_batch(stage, root / "derived" / "validation-fixture", ["first", "second"], rows, repo_root=root)
    ...
    def mock_execution(self, failure=None):
        ...
        def start_sim(runtime_root, shadow, config_path, binding_path, output_root):
            ...
            raw.mkdir()
            interrupted = failure in ("interrupt", "missing_terminal") and events.count("simulation") == 2
            ...
            meta = {"run_id": run_id, "status": "aborted" if interrupted else "completed",
                    ...
                    "logical_llm_calls": calls, "http_attempts": calls}
            if failure != "missing_terminal" or not interrupted:
        ...
        with tempfile.TemporaryDirectory() as temp, ExitStack() as stack:
            ...
            expected_simulations = 0 if failure == "probe" else 2 if failure in ("interrupt", "missing_terminal") else 6
            ...
            expected_attempts = (9 if failure == "probe" else 2985 if failure == "interrupt"
                                 else None if failure == "missing_terminal" else 17289)
            ...
            if failure == "interrupt":
                self.assertEqual([row["status"] for row in result["runs"]],
                                 ["completed", "aborted", "not_started", "not_started", "not_started", "not_started"])
            ...
            if failure == "missing_terminal":
                self.assertEqual(result["runs"][1]["status"], "incomplete_missing_terminal_metadata")
                self.assertNotIn("http_attempts", result["runs"][1])
                self.assertEqual(result["total_http_attempts_accounted"], 2889)
