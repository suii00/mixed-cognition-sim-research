"""Compact Japanese prompt contract with left before right presentation."""
...
3. `action`="move", `direction`="left"（左へ1マス）
4. `action`="move", `direction`="right"（右へ1マス）
...
def build_phase3_prompt(
    ...
    if contract_version != COMPACT_LR_RESPONSE_CONTRACT_VERSION:
        raise ValueError(
            "left-before-right compact prompt requires its paired response contract"
        )
