"""Versioned Japanese prompts for the prospective Japanese-model condition."""
...
近くのエージェントへ送るメッセージを決めてください。伝えることがなければ空文字列を送れます。
自然言語の値は日本語で記述し、JSONのキーは次に示す英字表記をそのまま使用してください。

JSONのみで応答してください:
{{"message": "<日本語のメッセージ>", "reasoning": "<判断理由の日本語説明>"}}\
"""
...
次の行動を決めてください。上下左右のいずれかへ1マス移動するか、その場に留まれます。
`action`と`direction`には次に示す英語の値を使用し、`memory`と`reasoning`は日本語で記述してください。

JSONのみで応答してください:
{{"action": "move" または "stay", "direction": "up" または "down" または "left" または "right", "memory": "<将来の自分への日本語のメモ>", "reasoning": "<判断理由の日本語説明>"}}\
"""
...
JSONのみで応答してください。`action`が"move"の場合、`direction`は"up"、"down"、"left"、"right"のいずれかでなければなりません。`action`が"stay"の場合、`direction`はnullでなければなりません:
{{"action": "move" または "stay", "direction": "up" または "down" または "left" または "right" または null, "memory": "<将来の自分への日本語のメモ>", "reasoning": "<判断理由の日本語説明>"}}\
"""
